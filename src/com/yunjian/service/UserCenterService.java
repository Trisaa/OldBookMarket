...
public class UserCenterService {
				...
				new QueryCompleteHandler(onQueryCompleteListener,GETMESSAGES) {
					...
					public void handleResponse(String jsonResult, EHttpError error) {
						...
						if(jsonResult!=null&&error==EHttpError.KErrorNone){
							...
							this.completeListener.onQueryComplete(GETMESSAGES, list, error);
						}
						else {
							this.completeListener.onQueryComplete(GETMESSAGES, null, error);
						}
					}
				});
	}
	 ...
	 * @return
	 */
	public void clearMessage(String userId,OnQueryCompleteListener onQueryCompleteListener){
		List<BasicNameValuePair>parms = new ArrayList<BasicNameValuePair>();
		parms.add(new BasicNameValuePair("user_id", userId));
		HttpUtils.makeAsyncPost(CLEARMESSAGEACTION, parms,
				new QueryCompleteHandler(onQueryCompleteListener, CLEARMESSAGE) {
					
					@Override
					public void handleResponse(String jsonResult, EHttpError error) {
						// TODO Auto-generated method stub
						if(jsonResult !=null&&error == EHttpError.KErrorNone){
							System.out.println("清空消息列表"+jsonResult);
							this.completeListener.onQueryComplete(CLEARMESSAGE, jsonResult, error);
						}
						else {
							this.completeListener.onQueryComplete(CLEARMESSAGE, null, error);
						}
					}
				});
	}
